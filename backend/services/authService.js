...
async function register(
  ...
  console.log("Register - fullName calculé:", fullName);

  if (!fullName) {
    const err = new Error("name is required (or both firstName and lastName)");
    err.status = 400;
    throw err;
  }
  if (!email) {
  ...
  try {
    const r = await db.runAsync(
      ...
      [fullName, email, password_hash, picture, role],
    );
