function mapPropertyRow(row) {
  ...
  return {
    ...
    host: row.host_id
      ? { id: row.host_id, name: row.host_name, picture: row.host_picture }
      : undefined,
  };
}
...
async function createProperty(db, payload) {
  ...
  console.log("host_id reçu:", host_id);
  console.log("host reçu:", host);
  console.log("Pictures reçues:", pictures);
  console.log(
    "Nombre de pictures:",
  ...
  console.log("resolvedHostId final:", resolvedHostId);
  if (!resolvedHostId) {
  ...
  if (Array.isArray(pictures)) {
    console.log("Insertion des pictures dans la DB...");
    for (const url of pictures) {
      if (url) {
        console.log("  - Insertion de:", url);
        await db.runAsync(
  ...
  const result = await getPropertyDetails(db, newId);
  console.log("Propriété créée avec", result.pictures?.length || 0, "pictures");
...
async function updateProperty(db, id, changes) {
  ...
  if (Array.isArray(changes.pictures)) {
    ...
    await db.runAsync("DELETE FROM property_pictures WHERE property_id = ?", [
      id,
    ]);
    // Insert new ones
  ...
  if (Array.isArray(changes.equipments)) {
    // Remove all existing equipments
    await db.runAsync("DELETE FROM property_equipments WHERE property_id = ?", [
      id,
    ]);
    // Insert new ones
...
async function getPropertyOwnerId(db, id) {
  const row = await db.getAsync("SELECT host_id FROM properties WHERE id = ?", [
    id,
  ]);
...
async function getAllEquipments(db) {
  const rows = await db.allAsync(
    "SELECT DISTINCT name FROM property_equipments ORDER BY name ASC",
  );
...
async function getAllTags(db) {
  const rows = await db.allAsync(
    "SELECT DISTINCT name FROM property_tags ORDER BY name ASC",
  );
  return rows.map((r) => r.name);
}
